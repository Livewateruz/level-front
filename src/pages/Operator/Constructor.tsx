...
function ConstructorOperator () {
    ...
    useEffect(() => {
        getData({
            url: `basedata/opr/constructor?page[offset]=${page}&${from ? `filter[start]=${from}` : ''}&${to ? `filter[end]=${to}` : ''}&${device ? `filter[device]=${device}` : ''}`,
            setData: setEvents,
            setLoading,
            token
        });
    ...
    const handleDownloadAll = () => {
        const fileDownloadUrl = `https://back2.livewater.uz/basedata/opr/constructorxlsx?${from ? `&filter[start]=${from}` : ''}${to ? `&filter[end]=${to}` : ''}${
                                ...
                                {events.data.map((data, i) => {
                                    return (
                                        <tr key={data._id}>
                                            ...
                                            <td className=''>
                                                <div className='whitespace-nowrap text-center '>961.8</div>
                                            </td>
                                            <td className=''>
...
export default ConstructorOperator;
